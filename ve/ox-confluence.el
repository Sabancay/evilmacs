...
(defun org-confluence-link (link desc info)
   (let ((raw-link (org-element-property :raw-link link)))
     (if (string-match "\\.\\(gif\\|png\\|jpg\\|jpeg\\)$" raw-link)
        (org-confluence-imp-image raw-link)
        (org-confluence-imp-link raw-link desc))
    )
)

(defun org-confluence-imp-image (raw-link)
    (concat "!" raw-link "!")
)

(defun org-confluence-imp-link (raw-link desc)
    (concat "["
